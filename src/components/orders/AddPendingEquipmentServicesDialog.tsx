...
interface AddPendingEquipmentServicesDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  equipmentName: string;
  onServicesAdded: (services: EquipmentService[]) => void;
}
...
export function AddPendingEquipmentServicesDialog({
  open,
  onOpenChange,
  equipmentName,
  onServicesAdded
}: AddPendingEquipmentServicesDialogProps) {
  ...
  const handleSubmit = () => {
    ...
    // Convert to EquipmentService format for pending equipment
    const equipmentServices: EquipmentService[] = selectedServices.flatMap(s =>
    ...
    onServicesAdded(equipmentServices);
    onOpenChange(false);

    toast({
      title: "Servicios agregados",
      description: `${equipmentServices.length} servicio(s) agregados al equipo`
    });
  };
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Agregar Servicios a: {equipmentName}
          </DialogTitle>
        </DialogHeader>
          ...
          {selectedServices.length > 0 && (
            <div className="border-t pt-4 space-y-3">
                ...
                {selectedServices.map((service, index) => (
                  <div key={index} className="flex items-center gap-2 p-2 bg-muted/50 rounded-lg">
                    ...
                    <span className="font-medium text-sm whitespace-nowrap">
                      {formatCOPCeilToTen(service.price * service.quantity)}
                    </span>
                    <Button
                      variant="ghost"
                      size="sm"
              ...
              <Button onClick={handleSubmit} className="w-full gap-2">
                <Plus className="h-4 w-4" />
                Agregar {selectedServices.length} servicio(s) al equipo
              </Button>
            </div>
          )}
        </div>
