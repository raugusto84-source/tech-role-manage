...
export function OrderModificationsBadge({ orderId, onChanged }: OrderModificationsBadgeProps) {
  ...
  const fetchMods = async () => {
    ...
    const { data, error } = await supabase
      ...
      .select("id, client_approved, created_at")
      .eq("order_id", orderId)
  ...
  const handleRejectLatest = async () => {
    ...
    try {
      ...
      if (data?.success) {
        toast({ title: "Actualización rechazada", description: "Se eliminaron los items agregados." });
        await fetchMods();
        onChanged?.();
      } else {
        toast({ title: "No se pudo rechazar", description: data?.error || "Intente nuevamente", variant: "destructive" });
      }
    } catch (e: any) {
      console.error("Reject error", e);
      toast({ title: "Error al rechazar", description: e?.message || "Intente nuevamente", variant: "destructive" });
    } finally {
      setRejecting(false);
    }
  };
