...
interface DiagnosticStep {
  ...
  solution_mapping?: {
    [key: string]: string;
  }; // mapea respuesta a solution_id
  next_step_mapping?: {
    [key: string]: string;
  }; // mapea respuesta a step_id
  image_url?: string;
}
interface Solution {
  id: string;
...
export function UnifiedDiagnosticManager() {
  ...
  useEffect(() => {
    const loadCategories = async () => {
      const {
        data
      } = await supabase.from('main_service_categories').select('id, name, icon').eq('is_active', true).order('name');
      setCategories(data || []);
    };
    loadCategories();
  }, []);
  ...
  useEffect(() => {
    const loadServiceTypes = async () => {
      const {
        data
      } = await supabase.from('service_types').select('id, name, category').eq('is_active', true).order('name');
  ...
  useEffect(() => {
    ...
    const loadFlows = async () => {
      const {
        data
      } = await supabase.from('diagnostic_flow').select(`
          id,
          category_id,
          problem_title,
          description,
        ...
        `).eq('category_id', selectedCategoryId).eq('is_active', true).order('problem_title');
      setFlows(data?.map(f => ({
        ...f,
        flow_data: f.flow_data as any || {
          steps: [],
          solutions: []
        },
  ...
  const handleCreateFlow = async () => {
    ...
    const {
      error
    } = await supabase.from('diagnostic_flow').insert({
  ...
  const handleDeleteFlow = async (flowId: string) => {
    ...
    const {
      error
    } = await supabase.from('diagnostic_flow').update({
      is_active: false
    }).eq('id', flowId);
    if (error) {
      toast({
  ...
  return <div className="space-y-6">
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 mt-2">
              {categories.map(category => <Button key={category.id} variant={selectedCategoryId === category.id ? 'default' : 'outline'} className="h-auto p-4 flex flex-col items-center gap-2" onClick={() => setSelectedCategoryId(category.id)}>
          ...
          {/* Create New Problem */}
          {selectedCategoryId && <div className="space-y-4 border-t pt-4">
              <h3 className="font-medium">Crear Nuevo Problema</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <Label>Título del Problema</Label>
                  <Input placeholder="Ej: Computadora lenta" value={newFlow.problem_title} onChange={e => setNewFlow(prev => ({
                ...prev,
                ...
                <div>
                  ...
                  <Input placeholder="Descripción breve del problema" value={newFlow.description} onChange={e => setNewFlow(prev => ({
                ...prev,
      ...
      {/* Problems List */}
      {selectedCategoryId && flows.length > 0 && <Card>
          <CardHeader>
            <CardTitle>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {flows.map(flow => <Card key={flow.id} className="relative border-l-4 border-l-primary">
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      ...
                      {flow.flow_data.solutions.length > 0 && <div className="text-xs text-muted-foreground">
                          <span className="font-medium">Solución principal:</span><br />
                          {flow.flow_data.solutions[0].title}
                        </div>}
                      
                      <div className="flex gap-2">
                        <Button size="sm" variant="outline" className="flex-1" onClick={() => handleConfigureFlow(flow)}>
                          <Settings className="h-4 w-4 mr-1" />
                          Configurar Diagrama
                        </Button>
                        <Button size="sm" variant="destructive" onClick={() => handleDeleteFlow(flow.id)} disabled={loading}>
      ...
      <VisualFlowEditor flow={editingFlow} isOpen={isEditorOpen} onClose={() => {
      setIsEditorOpen(false);
      setEditingFlow(null);
    }} onSave={handleEditorSave} serviceTypes={serviceTypes} />
    </div>;
}
...
function VisualFlowEditor({
  ...
  const updateStep = (stepId: string, updates: Partial<DiagnosticStep>) => {
    ...
    setEditingFlow({
      ...
      flow_data: {
        ...editingFlow.flow_data,
        steps: editingFlow.flow_data.steps.map(step => step.id === stepId ? {
          ...step,
          ...updates
        } : step)
      }
  ...
  const updateSolution = (solutionId: string, updates: Partial<Solution>) => {
    ...
    setEditingFlow({
      ...
      flow_data: {
        ...editingFlow.flow_data,
        solutions: editingFlow.flow_data.solutions.map(solution => solution.id === solutionId ? {
          ...solution,
          ...updates
        } : solution)
      }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-4 max-h-[400px] overflow-y-auto">
              {editingFlow.flow_data.steps.map((step, index) => <Card key={step.id} className="border-l-4 border-l-blue-500">
                  ...
                  <CardContent className="space-y-3">
                    <div>
                      ...
                      <Input value={step.question} onChange={e => updateStep(step.id, {
                    question: e.target.value
                  })} placeholder="¿Cuál es el problema?" className="text-sm" />
                    </div>
                    
                    <div>
                      <Label className="text-xs">Tipo</Label>
                      <Select value={step.type} onValueChange={(value: 'yes_no' | 'multiple_choice' | 'text') => updateStep(step.id, {
                    type: value
                  })}>
                    ...
                    {step.type === 'multiple_choice' && <div>
                        <Label className="text-xs">Opciones</Label>
                        <Textarea value={step.options?.join('\n') || ''} onChange={e => updateStep(step.id, {
                    options: e.target.value.split('\n').filter(opt => opt.trim())
                  })} placeholder="Opción 1&#10;Opción 2&#10;Opción 3" className="text-sm" rows={3} />
                      </div>}
                      ...
                      <div>
                        ...
                        <div className="space-y-3">
                          {step.type === 'yes_no' ? <div className="space-y-2">
                              <div className="text-xs font-medium">Respuesta: Sí</div>
                              <div className="grid grid-cols-2 gap-2">
                                <div>
                                  <Label className="text-xs">Ir a pregunta:</Label>
                                  <Select value={step.next_step_mapping?.['Sí'] || ''} onValueChange={stepId => updateStep(step.id, {
                            next_step_mapping: {
                              ...
                              'Sí': stepId
                                    ...
                                    <SelectContent>
                                      {editingFlow?.flow_data.steps.filter(s => s.id !== step.id).map(otherStep => <SelectItem key={otherStep.id} value={otherStep.id}>
                                ...
                                <div>
                                  ...
                                  <Select value={step.solution_mapping?.['Sí'] || ''} onValueChange={solutionId => updateStep(step.id, {
                            solution_mapping: {
                              ...step.solution_mapping,
                              'Sí': solutionId
                            }
                          })}>
                                    <SelectTrigger className="text-xs">
                                    ...
                                    <SelectContent>
                                      {editingFlow?.flow_data.solutions.map(solution => <SelectItem key={solution.id} value={solution.id}>
                              ...
                              <div className="grid grid-cols-2 gap-2">
                                <div>
                                  <Label className="text-xs">Ir a pregunta:</Label>
                                  <Select value={step.next_step_mapping?.['No'] || ''} onValueChange={stepId => updateStep(step.id, {
                            next_step_mapping: {
                              ...step.next_step_mapping,
                              'No': stepId
                            }
                                ...
                                <div>
                                  <Label className="text-xs">O ir a solución:</Label>
                                  <Select value={step.solution_mapping?.['No'] || ''} onValueChange={solutionId => updateStep(step.id, {
                            solution_mapping: {
                              ...step.solution_mapping,
                              'No': solutionId
                            }
                            ...
                            </div> : step.type === 'multiple_choice' && step.options ? <div className="space-y-2 max-h-40 overflow-y-auto">
                              {step.options.map(option => <div key={option} className="border-b pb-2">
                                  <div className="text-xs font-medium mb-1">{option}:</div>
                                  <div className="grid grid-cols-2 gap-2">
                                    <div>
                                      <Select value={step.next_step_mapping?.[option] || ''} onValueChange={stepId => updateStep(step.id, {
                              next_step_mapping: {
                                ...step.next_step_mapping,
                                [option]: stepId
                              }
                                    ...
                                    <div>
                                      <Select value={step.solution_mapping?.[option] || ''} onValueChange={solutionId => updateStep(step.id, {
                              solution_mapping: {
                                ...step.solution_mapping,
                                [option]: solutionId
                              }
          ...
          <Card>
            ...
            <CardContent className="space-y-4 max-h-[400px] overflow-y-auto">
              {editingFlow.flow_data.solutions.map((solution, index) => <Card key={solution.id} className="border-l-4 border-l-green-500">
                  <CardHeader className="pb-2">
                    <div className="flex items-center justify-between">
                  ...
                  <CardContent className="space-y-3">
                    <div>
                      ...
                      <Input value={solution.title} onChange={e => updateSolution(solution.id, {
                    title: e.target.value
                  })} placeholder="Nombre de la solución" className="text-sm" />
                    </div>
                    
                    <div>
                      <Label className="text-xs">Descripción</Label>
                      <Textarea value={solution.description} onChange={e => updateSolution(solution.id, {
                    description: e.target.value
                  })} placeholder="Descripción de la solución..." className="text-sm" rows={2} />
                    </div>
                    ...
                    if (!currentServices.includes(serviceId)) {
                      ...
                      {solution.recommended_services.length > 0 && <div className="flex flex-wrap gap-1 mt-2">
                          {solution.recommended_services.map(serviceId => {
                      const service = serviceTypes.find(s => s.id === serviceId);
                      return service ? <Badge key={serviceId} variant="outline" className="text-[10px] px-1 py-0">
                                {service.name}
                                <Button size="sm" variant="ghost" className="h-auto p-0 ml-1" onClick={() => updateSolution(solution.id, {
                    ...
                    <div>
                      ...
                      <Input type="number" min="0" max="100" value={solution.confidence_score} onChange={e => updateSolution(solution.id, {
