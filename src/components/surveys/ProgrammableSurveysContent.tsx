...
interface ProgrammableSurveysContentProps {
  surveyConfigs: SurveyConfig[];
  scheduledSurveys: ScheduledSurvey[];
  showConfigDialog: boolean;
  setShowConfigDialog: (show: boolean) => void;
  editingConfig: SurveyConfig | null;
  setEditingConfig: (config: SurveyConfig | null) => void;
  formData: any;
  setFormData: (data: any) => void;
  onReload: () => void;
}

export function ProgrammableSurveysContent({
  surveyConfigs,
  scheduledSurveys,
  showConfigDialog,
  setShowConfigDialog,
  ...
  setFormData,
  onReload
}: ProgrammableSurveysContentProps) {
  const { user } = useAuth();
  ...
  const addQuestion = () => {
    const newId = Math.max(...formData.survey_questions.map((q: any) => q.id), 0) + 1;
    setFormData({
      ...formData,
      survey_questions: [
  ...
  const updateQuestion = (id: number, field: string, value: any) => {
    setFormData({
      ...formData,
      survey_questions: formData.survey_questions.map((q: any) =>
  ...
  const removeQuestion = (id: number) => {
    setFormData({
      ...formData,
      survey_questions: formData.survey_questions.filter((q: any) => q.id !== id)
    });
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-bold text-foreground">Encuestas Programables</h2>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="space-y-3">
                  {formData.survey_questions.map((question: any, index: number) => (
        ...
        <CardContent>
                ...
                <TableRow key={config.id}>
                  ...
                  <TableCell>
                    {config.is_active ? (
                      ...
                      <Badge variant="outline" className="bg-muted/10">Inactiva</Badge>
                    )}
                  </TableCell>
                  <TableCell>
          ...
          {surveyConfigs.length === 0 && (
            <div className="text-center py-8 text-muted-foreground">
              No hay configuraciones de encuestas. Crea una nueva configuración para comenzar.
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Encuestas Programadas</CardTitle>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
            ...
            <TableBody>
              {scheduledSurveys.map((survey) => (
                <TableRow key={survey.id}>
                  ...
                  <TableCell>{survey.client_email}</TableCell>
                  <TableCell>
          ...
          {scheduledSurveys.length === 0 && (
            <div className="text-center py-8 text-muted-foreground">
              No hay encuestas programadas actualmente.
            </div>
          )}
        </CardContent>
      </Card>
