...
import { Plus, FileText, ClipboardList, Signature, CheckCircle, Clock, AlertCircle, Star, Gift, ArrowRight, Zap, TrendingUp, CalendarDays } from "lucide-react";
...
export default function ClientDashboard() {
  ...
  const loadQuotes = async () => {
    ...
    const {
      data,
      error
    } = await supabase.from("quotes").select("*").eq("client_email", profile.email).order("created_at", {
      ascending: false
    }).limit(5);
    if (error) {
  ...
  const loadPendingApprovalQuotes = async () => {
    ...
    const {
      data,
      error
    } = await supabase.from('quotes').select('*').eq('client_email', profile.email).in('status', ['solicitud', 'enviada', 'pendiente_aprobacion']) // Todas las cotizaciones nuevas
    .order('created_at', {
      ascending: false
    });
    if (error) {
  ...
  const loadRewards = async () => {
    ...
    try {
      ...
      const {
        data: rewardsData
      } = await supabase.from('client_rewards').select('*').eq('client_id', client.id).single();
      const {
        data: referralData
      } = await supabase.from('client_referrals').select('referral_code').eq('referrer_client_id', client.id).single();
      setRewards({
  ...
  const loadOrders = async () => {
    ...
    const {
      data: client,
      error: clientErr
    } = await supabase.from("clients").select("id").eq("user_id", profile.user_id).maybeSingle();
    ...
    if (clientErr || !client) {
      ...
      const {
        data: clientByEmail,
        error: emailErr
      } = await supabase.from("clients").select("id").eq("email", profile.email).maybeSingle();
      ...
      const {
        data: ordersData,
        error
      } = await supabase.from("orders").select("*").eq("client_id", clientByEmail.id).order("created_at", {
        ascending: false
      });
      ...
      const ordersWithTechnicianNames = await Promise.all((ordersData || []).map(async (order: any) => {
        if (order.assigned_technician) {
          const {
            data: techProfile
          } = await supabase.from('profiles').select('full_name').eq('user_id', order.assigned_technician).maybeSingle();
          return {
            ...order,
    ...
    const {
      data: ordersData,
      error
    } = await supabase.from("orders").select("*").eq("client_id", client.id).order("created_at", {
      ascending: false
    });
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      await Promise.all([loadOrders(), loadQuotes(), loadPendingApprovalQuotes(), loadRewards()]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!client) return;
      channel = supabase.channel("client-orders-realtime").on("postgres_changes", {
        event: "*",
        schema: "public",
        table: "orders",
  ...
  useEffect(() => {
    if (!profile?.email) return;
    const quotesChannel = supabase.channel('client-quotes-realtime').on('postgres_changes', {
      event: '*',
      schema: 'public',
      table: 'quotes',
  ...
  const metrics = useMemo(() => ({
    pendingOrders: orders.filter(o => ['pendiente', 'en_proceso', 'en_camino'].includes(o.status)).length,
    ...
    activeQuotes: quotes.filter(q => ['solicitud', 'enviada'].includes(q.status)).length,
  ...
  return <AppLayout>
      <div className="max-w-md mx-auto space-y-4">
        ...
        {/* Alertas importantes */}
        {metrics.pendingApproval > 0 && <Card className="border-red-200 bg-red-50">
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                ...
                <Button size="sm" variant="outline" className="border-red-200 text-red-600" onClick={() => pendingApprovalOrders.length > 0 && handleApproveOrder(pendingApprovalOrders[0].id)}>
                  Autorizar
                </Button>
              </div>
            </CardContent>
        ...
        {metrics.quotesToApprove > 0 && <Card className="border-blue-200 bg-blue-50">
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
        ...
        {metrics.readyToSign > 0 && <Card className="border-orange-200 bg-orange-50">
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                ...
                <Button size="sm" variant="outline" className="border-orange-200 text-orange-600" onClick={() => readyForSignatureOrders.length > 0 && setOrderToSign(readyForSignatureOrders[0])}>
                  Firmar
                </Button>
              </div>
            </CardContent>
        ...
        {metrics.pendingUpdates > 0 && <Card className="border-purple-200 bg-purple-50">
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                ...
                <Button size="sm" variant="outline" className="border-purple-200 text-purple-600" onClick={() => pendingUpdateOrders.length > 0 && handleViewOrderDetails(pendingUpdateOrders[0].id)}>
                  Ver
                </Button>
              </div>
            </CardContent>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <Card className="bg-gradient-to-r from-yellow-50 to-orange-50 border-yellow-200 h-24">
            <CardContent className="p-3 h-full flex items-center justify-between">
              ...
              <Button size="sm" variant="outline" className="border-yellow-300 text-yellow-700" onClick={() => setShowCashbackHistory(true)}>
                <Star className="h-3 w-3" />
              </Button>
            </CardContent>
          </Card>
          
          <Button onClick={() => window.location.href = '/quotes'} variant="outline" className="h-24 flex-col gap-2 border-2" size="lg">
        ...
        <Card>
          ...
          <CardContent className="space-y-2 pt-0">
            {pendingApprovalOrders.length === 0 ? <p className="text-sm text-muted-foreground">No tienes órdenes pendientes de autorización.</p> : pendingApprovalOrders.slice(0, 3).map(order => <div key={order.id} className="flex items-center justify-between p-3 rounded-lg border bg-red-50">
                  <div className="flex items-center gap-3">
                  ...
                  <Button size="sm" className="bg-red-600 hover:bg-red-700 text-white" onClick={() => handleApproveOrder(order.id)}>
                    Autorizar
                  </Button>
                </div>)}
          </CardContent>
        ...
        <Card>
          ...
          <CardContent className="space-y-2 pt-0">
            {pendingApprovalQuotes.length === 0 ? <p className="text-sm text-muted-foreground">No tienes cotizaciones por aprobar.</p> : pendingApprovalQuotes.slice(0, 3).map(quote => <div key={quote.id} className="flex items-center justify-between p-3 rounded-lg border bg-blue-50">
                  <div className="flex items-center gap-3">
                    <FileText className="h-4 w-4 text-blue-600" />
                    <div>
        ...
        <Card>
          ...
          <CardContent className="space-y-2 pt-0">
            {readyForSignatureOrders.length === 0 ? <p className="text-sm text-muted-foreground">No hay servicios listos para firma.</p> : readyForSignatureOrders.slice(0, 3).map(order => <div key={order.id} className="flex items-center justify-between p-3 rounded-lg border bg-orange-50">
                  <div className="flex items-center gap-3">
                    <Signature className="h-4 w-4 text-orange-600" />
                    <div>
                      <p className="text-sm font-medium">{order.order_number}</p>
                  ...
                  <Button size="sm" className="bg-orange-600 hover:bg-orange-700 text-white" onClick={() => setOrderToSign(order)}>
                    Firmar
                  </Button>
                </div>)}
          </CardContent>
        ...
        <Card>
          ...
          <CardContent className="space-y-2 pt-0">
            {pendingUpdateOrders.length === 0 ? <p className="text-sm text-muted-foreground">No hay actualizaciones recientes.</p> : pendingUpdateOrders.slice(0, 3).map(order => <div key={order.id} className="flex items-center justify-between p-3 rounded-lg border bg-purple-50">
                  <div className="flex items-center gap-3">
                    <Zap className="h-4 w-4 text-purple-600" />
                    <div>
                      <p className="text-sm font-medium">{order.order_number}</p>
        ...
        <Card>
          ...
          <CardContent className="space-y-3 pt-0">
            {[...orders.slice(0, 2), ...quotes.slice(0, 2)].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()).slice(0, 3).map((item: any) => <div key={item.id} className="flex items-center justify-between p-2 rounded-lg bg-muted/30">
                  <div className="flex items-center gap-2">
                    {'order_number' in item ? <ClipboardList className="h-4 w-4 text-primary" /> : <FileText className="h-4 w-4 text-green-600" />}
                    <div>
            ...
            {orders.length === 0 && quotes.length === 0 && <div className="text-center py-6">
        ...
        {/* Enlaces rápidos */}
        <div className="grid grid-cols-2 gap-3">
          <Button variant="ghost" className="h-12 justify-between" onClick={() => handleViewAll('orders')}>
            <div className="flex items-center gap-2">
          ...
          <Button variant="ghost" className="h-12 justify-between" onClick={() => handleViewAll('quotes')}>
            <div className="flex items-center gap-2">
              <FileText className="h-4 w-4" />
      ...
      {/* Diálogos */}
      {orderToSign && <DeliverySignature order={{
      id: orderToSign.id,
      ...
      {showNewRequestDialog && <NewRequestDialog open={showNewRequestDialog} onOpenChange={open => {
      setShowNewRequestDialog(open);
    }} />}

      {/* Cashback History Modal */}
      <ClientCashbackHistory open={showCashbackHistory} onOpenChange={setShowCashbackHistory} />
    </AppLayout>;
}