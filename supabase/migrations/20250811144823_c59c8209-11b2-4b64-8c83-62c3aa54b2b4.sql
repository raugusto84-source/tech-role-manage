-- Fix the enum value in the trigger
...
BEGIN
  ...
  IF NEW.status = 'aceptada' AND (OLD.status IS NULL OR OLD.status != 'aceptada') THEN
    ...
    RAISE LOG 'About to create order with status pendiente_aprobacion';
    
    -- Crear la orden con status pendiente_aprobacion para que requiera aprobación del cliente
    INSERT INTO public.orders (
      ...
      NEW.assigned_to,
      NULL,
      'pendiente_aprobacion'::order_status,  -- Fix: Use correct enum value
      NULL  -- Cliente aún no ha aprobado
    ...
    RAISE LOG 'Order % created successfully with total % and status pendiente_aprobacion', order_record.order_number, order_total;
