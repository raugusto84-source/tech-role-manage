-- Add hourly rate field to work_schedules table for payroll calculation
ALTER TABLE public.work_schedules ADD COLUMN IF NOT EXISTS hourly_rate NUMERIC DEFAULT 0;
ALTER TABLE public.work_schedules ADD COLUMN IF NOT EXISTS overtime_rate NUMERIC DEFAULT 0;

-- Create optimized view for admin employee reports
CREATE OR REPLACE VIEW public.admin_employee_overview AS
SELECT 
    p.user_id,
    p.full_name,
    p.email,
...
GRANT SELECT ON public.admin_employee_overview TO authenticated;

-- Create function to calculate weekly payroll for an employee
