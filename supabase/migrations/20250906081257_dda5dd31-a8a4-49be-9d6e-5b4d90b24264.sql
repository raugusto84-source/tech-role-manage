-- Update suggest_optimal_fleet function to use service_category field
...
BEGIN
  -- Get the service category directly from service_types table
  SELECT st.service_category INTO service_category
  FROM public.service_types st 
  WHERE st.id = p_service_type_id;
  
  -- If service category not found, default to sistemas
  ...
  FOR rec IN 
    ...
    AND fg.category = service_category  -- Filter by service category from service_types
