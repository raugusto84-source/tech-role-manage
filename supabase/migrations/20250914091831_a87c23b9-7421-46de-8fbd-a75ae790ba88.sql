-- Update notify_quote_status_change function to handle new status
  ...
  IF TG_OP = 'UPDATE' AND OLD.status IS DISTINCT FROM NEW.status THEN
    CASE NEW.status
      ...
      WHEN 'pendiente_aprobacion' THEN
        ...
        message_content := 'Su cotización #' || NEW.quote_number || ' está pendiente de aprobación. Le notificaremos cuando sea procesada.';
      WHEN 'aceptada' THEN
...
-- Update create_sales_survey function to handle new quote status
...
DECLARE
  client_user_id UUID;
BEGIN
  -- Create survey when status changes to 'aceptada' (accepted quotes)
