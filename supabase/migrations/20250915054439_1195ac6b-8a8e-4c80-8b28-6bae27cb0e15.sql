-- Update clean_workflow_data to also remove pending collections and policy payments
...
BEGIN
  ...
  -- 0. Remove pending collections and policy payments first to avoid orphans
  DELETE FROM public.pending_collections WHERE id IS NOT NULL;
  GET DIAGNOSTICS deleted_count = ROW_COUNT;
  RAISE LOG 'Deleted % pending collections', deleted_count;
  
  DELETE FROM public.policy_payments WHERE id IS NOT NULL;
  GET DIAGNOSTICS deleted_count = ROW_COUNT;
  RAISE LOG 'Deleted % policy payments', deleted_count;
  ...
  -- Preserve base/master data (clients, catalog, configs, profiles, etc.)
  
  RETURN json_build_object(
    'success', true,
    'message', 'Flujo de trabajo limpiado exitosamente. Se eliminaron órdenes, cotizaciones, finanzas y cobranzas pendientes.'
  );
