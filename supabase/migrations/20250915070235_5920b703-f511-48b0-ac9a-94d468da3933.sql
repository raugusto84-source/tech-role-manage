-- Drop triggers first, then recreate the function with the correct calculation
...
DROP FUNCTION IF EXISTS public.manage_pending_collections() CASCADE;
DROP FUNCTION IF EXISTS public.update_pending_collections_on_items() CASCADE;
...
AS $function$
BEGIN
  -- Refresh pending collections when order items are modified
  PERFORM public.manage_pending_collections();
...
CREATE TRIGGER trg_update_pending_collections_on_items
  AFTER INSERT OR UPDATE OR DELETE ON public.order_items
  FOR EACH ROW
  EXECUTE FUNCTION public.update_pending_collections_on_items();

-- Execute the function to refresh current data
SELECT public.manage_pending_collections();