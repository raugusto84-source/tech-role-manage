-- Add a rebuild function and update the trigger function to compute totals by rounding each item, then sum
...
BEGIN
  ...
  FOR order_rec IN 
    SELECT o.id, o.order_number, o.client_id, c.name AS client_name, c.email AS client_email, o.created_at
...
-- Keep the same name/signature for existing triggers; just call the rebuild
...
-- Run once to refresh current data
