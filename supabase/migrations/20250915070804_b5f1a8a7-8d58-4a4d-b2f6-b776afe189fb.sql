-- Update pending collections to include all orders that were once finalized, regardless of current status
...
BEGIN
  ...
  -- Include all orders that were EVER finalized (check status logs), not just currently finalized
  FOR order_rec IN 
  ...
  LOOP
    ...
    WHERE client_id = order_rec.client_id
      AND EXISTS (
        SELECT 1 FROM public.order_status_logs osl2 
        WHERE osl2.order_id = orders.id 
        AND osl2.new_status = 'finalizada'
      )
