-- Align server totals with UI "Total General (Visual)" logic (retry with lateral update)
...
-- 2) Recalculate all existing order_totals to reflect the new logic (use LATERAL to reference ot)
UPDATE public.order_totals AS ot
SET 
  ...
  calculated_at = now(),
  updated_at = now()
FROM LATERAL public.calculate_order_total(ot.order_id) AS calc(subtotal numeric, vat_amount numeric, total_amount numeric);
