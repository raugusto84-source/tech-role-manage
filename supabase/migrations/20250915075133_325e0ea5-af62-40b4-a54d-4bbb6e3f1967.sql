-- Retry: update function and recalc totals using join on orders to avoid referencing target alias in function call
...
BEGIN
  ...
  -- UI applies ONLY general_cashback_percent when apply_cashback_to_items=true
...
-- Recalculate order_totals by joining through orders and lateral function
...
SET 
  ...
  calculated_at = now(),
  updated_at = now()
FROM public.orders o, LATERAL public.calculate_order_total(o.id) AS calc(subtotal numeric, vat_amount numeric, total_amount numeric)
WHERE ot.order_id = o.id;

-- Refresh pending collections with updated totals
