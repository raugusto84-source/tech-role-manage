-- Align server totals with UI "Total General (Visual)" logic (update + recalc via DO loop)
...
-- 2) Recalculate all existing order_totals to reflect the new logic (DO loop to avoid alias scope issues)
DO $$
DECLARE
  rec RECORD;
  calc RECORD;
