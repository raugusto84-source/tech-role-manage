-- Update auto_complete_order function to prevent duplicate cashback transactions
...
DECLARE
  ...
  order_total NUMERIC;
  client_info RECORD;
  existing_cashback_transaction RECORD;
BEGIN
  -- Log para debugging
  ...
  IF TG_OP = 'UPDATE' AND NEW.status = 'finalizada' AND OLD.status != 'finalizada' THEN
    ...
    IF total_items = completed_items THEN
      ...
      -- Check if cashback transaction already exists for this order to prevent duplicates
      SELECT * INTO existing_cashback_transaction
      ...
      -- Only process if no existing cashback transaction
      IF existing_cashback_transaction.id IS NULL THEN
        ...
        END IF;
      ELSE
        RAISE LOG 'Cashback already processed for order %, skipping duplicate', NEW.order_id;
      END IF;
    ELSE
