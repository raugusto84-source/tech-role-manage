-- Fix convert_quote_to_order to properly handle cashback by adjusting order item prices proportionally
...
BEGIN
  ...
  -- Verificar por quote_number en las notas del historial
  SELECT o.* INTO existing_order
  FROM public.orders o
  ...
  -- Si hay cashback aplicado, calcular el factor de descuento
  ...
  -- Crear la orden con el total de la cotización (ya incluye descuento por cashback)
  INSERT INTO public.orders (
    ...
    quote_record.estimated_amount, -- Total exacto de la cotización (con descuento)
  ...
  SELECT 
    ...
    qi.description,
    qi.quantity,
    -- Ajustar precios unitarios con el factor de descuento
    (qi.subtotal * discount_factor / GREATEST(qi.quantity, 1)),
    (qi.subtotal * discount_factor / GREATEST(qi.quantity, 1)),
    0, -- Sin margen, precio fijo de cotización
    qi.subtotal * discount_factor, -- Aplicar descuento proporcional
    qi.vat_rate,
    qi.vat_amount * discount_factor, -- Aplicar descuento proporcional al IVA
    qi.total * discount_factor, -- Aplicar descuento proporcional al total
  ...
  -- Registrar en historial CON REFERENCIA ESPECÍFICA AL QUOTE_NUMBER
