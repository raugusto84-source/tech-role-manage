-- =============================================================================
-- DATABASE OPTIMIZATION FOR AUTOMATION SYSTEM
...
LEFT JOIN public.policies pol ON pol.id = pp.policy_id
...
SELECT 
  ...
  -- Time metrics
  EXTRACT(EPOCH FROM (o.updated_at - o.created_at))/3600 as completion_hours,
...
GROUP BY o.id, c.name, c.email, tp.full_name, sp.full_name;
...
SELECT 
  ...
  COALESCE(COUNT(CASE WHEN ss.status = 'completed' THEN 1 END), 0) as completed_services,
  COALESCE(COUNT(o.id), 0) as generated_orders,
  -- Revenue metrics
  p.monthly_cost * EXTRACT(MONTHS FROM AGE(COALESCE(p.end_date, CURRENT_DATE), p.start_date)) as projected_revenue,
  -- Health score (0-100)
  CASE 
    ...
    WHEN COALESCE(COUNT(CASE WHEN ss.status = 'completed' THEN 1 END), 0) * 100.0 / NULLIF(COUNT(ss.id), 0) >= 80 THEN 100
    ELSE 75
  END as health_score
FROM public.policies p
LEFT JOIN public.policy_payments pp ON pp.policy_id = p.id
LEFT JOIN public.scheduled_services ss ON ss.policy_id = p.id
LEFT JOIN public.orders o ON o.id = ss.order_id
...
SELECT 
  ...
  -- Performance metrics
  COALESCE(AVG(EXTRACT(EPOCH FROM (o.updated_at - o.created_at))/3600), 0) as avg_completion_hours,
  ...
  -- Skills metrics
  COUNT(ts.id) as total_skills,
  COALESCE(AVG(ts.skill_level), 0) as avg_skill_level,
  -- Workload metrics
  COUNT(CASE WHEN tw.status = 'assigned' THEN 1 END) as current_workload
...
LEFT JOIN public.technician_workload tw ON tw.technician_id = p.user_id
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_policies_automation 
  ON public.policies(status, start_date, end_date) 
  WHERE status = 'activa';

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_policy_payments_automation 
  ON public.policy_payments(policy_id, payment_date, status);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_scheduled_services_automation 
  ON public.scheduled_services(policy_id, next_service_date, status) 
  WHERE status = 'active';

-- Indexes for financial queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_incomes_date_amount 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_financial_projections_period 
  ON public.financial_projections(projection_month, projection_year);
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_order_items_order_type 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_technician_skills_performance 
  ON public.technician_skills(technician_id, skill_level, service_type_id);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_technician_workload_status 
  ON public.technician_workload(technician_id, status, created_at);

-- Indexes for client and communication
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_clients_email_status 
  ON public.clients(email, created_at DESC);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_follow_up_reminders_scheduled 
  ON public.follow_up_reminders(scheduled_at, status) 
  WHERE status = 'pending';

-- Composite indexes for complex automation queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_orders_client_technician_status 
  ON public.orders(client_id, assigned_technician, status, created_at);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_policies_client_status_dates 
