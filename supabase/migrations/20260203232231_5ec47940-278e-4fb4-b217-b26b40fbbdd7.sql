-- Fix the convert_quote_to_order function to correctly map quote_items.total to order_items.total_amount
-- The quote_items.total already includes quantity, so we should NOT multiply again
...
BEGIN
    ...
    LOOP
        INSERT INTO order_items (
            ...
            v_item.unit_price,
            COALESCE((v_item.unit_price - COALESCE(v_item.cost_price, 0)) / NULLIF(v_item.unit_price, 0) * 100, 30),
            v_item.subtotal,
            v_item.vat_rate,
            v_item.vat_amount,
            v_item.total,  -- Use total directly, it already includes quantity
